 ...
 * sub-process. It depends on the javax.sound.* packages and does not work on
 * Android.
 * 
 * @author Joren Six
 * @see AudioDispatcher
...
public class AudioDispatcherFactory {
	...
	/**
	 * Create a new AudioDispatcher connected to the default microphone. The
	 * default is defined by the Java runtime by calling
	 * 
	 * <pre>
	 * AudioSystem.getTargetDataLine(format)
	 * </pre>
	 * 
	 * . The microphone must support the format: 44100Hz sample rate, 16bits
	 * mono, signed big endian.
	 * 
	 ...
	 */
	public static AudioDispatcher fromDefaultMicrophone(final int audioBufferSize, final int bufferOverlap)
			throws LineUnavailableException {
	 ...
	 * </pre>
	 * 
	 * . The microphone must support the format of the requested sample rate,
	 * 16bits mono, signed big endian.
	 * 
	 * @param sampleRate
	 *            The <b>requested</b> sample rate must be supported by the
	 *            capture device. Nonstandard sample rates can be problematic!
	 * @param audioBufferSize
	 *            The size of the buffer defines how much samples are processed
	 *            in one step. Common values are 1024,2048.
	 * @param bufferOverlap
	 ...
	 * @throws LineUnavailableException
	 */
	public static AudioDispatcher fromDefaultMicrophone(final int sampleRate, final int audioBufferSize,
			final int bufferOverlap) throws LineUnavailableException {
	 ...
	 */
	public static AudioDispatcher fromPipe(final String source, final int targetSampleRate, final int audioBufferSize,
	 ...
	 */
	public static AudioDispatcher fromPipe(final String source, final int targetSampleRate, final int audioBufferSize,
			final int bufferOverlap, final double startTimeOffset) {
	 ...
	 */
	public static AudioDispatcher fromPipe(final String source, final int targetSampleRate, final int audioBufferSize,
			final int bufferOverlap, final double startTimeOffset, final double numberOfSeconds) {
		if (new File(source).exists() && new File(source).isFile() && new File(source).canRead()) {
			PipedAudioStream f = new PipedAudioStream(source);
	 ...
	 * @param sampleRate
	 *            The sample rate of the audio information contained in the
	 *            buffer.
	 * @param audioBufferSize
	 *            The size of the buffer defines how much samples are processed
	 *            in one step. Common values are 1024,2048.
	 ...
	 *             If the audio format is not supported.
	 */
	public static AudioDispatcher fromFloatArray(final float[] floatArray, final int sampleRate,
			final int audioBufferSize, final int bufferOverlap) throws UnsupportedAudioFileException {
		...
		final TarsosDSPAudioFloatConverter converter = TarsosDSPAudioFloatConverter
				.getConverter(JVMAudioInputStream.toTarsosDSPFormat(audioFormat));
