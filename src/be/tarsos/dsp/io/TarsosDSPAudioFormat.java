...
public class TarsosDSPAudioFormat {
	 ...
	 */
	public TarsosDSPAudioFormat(Encoding encoding, float sampleRate, int sampleSizeInBits, int channels, int frameSize,
	 ...
	 *            big-endian byte order (<code>false</code> means little-endian)
	 */
	public TarsosDSPAudioFormat(float sampleRate, int sampleSizeInBits, int channels, boolean signed,
			boolean bigEndian) {

		this((signed == true ? Encoding.PCM_SIGNED : Encoding.PCM_UNSIGNED), sampleRate, sampleSizeInBits, channels,
				(channels == NOT_SPECIFIED || sampleSizeInBits == NOT_SPECIFIED) ? NOT_SPECIFIED
	 ...
	 * or capabilities , a return value of
	 * <code>AudioSystem.NOT_SPECIFIED</code> means that any (positive) number
	 ...
	 * queries or capabilities, a frame size of
	 ...
	 * used for queries or capabilities , a frame rate of
	 ...
	 * sample rate value <code>AudioSystem.NOT_SPECIFIED</code>, which any
	 * sample rate will match. The frame rates must similarly be equal, unless
	 * the specified format has the frame rate value
	 * <code>AudioSystem.NOT_SPECIFIED</code>. The byte order (big-endian or
	...
	public boolean matches(TarsosDSPAudioFormat format) {
		...
		if (format.getEncoding().equals(getEncoding())
				...
				&& (format.getChannels() == getChannels() && (format.getFrameSize() == getFrameSize())
	...
	public String toString() {
		...
		if ((getEncoding().equals(Encoding.PCM_SIGNED) || getEncoding().equals(Encoding.PCM_UNSIGNED))
				&& ((getSampleSizeInBits() > 8) || (getSampleSizeInBits() == NOT_SPECIFIED))) {
			if (isBigEndian()) {
				sEndian = "big-endian";
			} else {
	 ...
	 * The numbers are frequently signed or unsigned integers. Besides PCM,
	 ...
	 * formats or floating-point PCM samples, and make these available through
	 * the <code>AudioSystem</code> class.
	 * <p>
