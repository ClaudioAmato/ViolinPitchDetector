...
public class AudioDispatcher implements Runnable {
	...
	private void skipToStart() {
		...
		try {
			...
			String message = String.format("Did not skip the expected amount of bytes,  %d skipped, %d expected!",
					skipped, bytesToSkip);
			LOG.warning(message);
			throw new Error(message);
		}
	...
	private int readNextAudioBlock() throws IOException {
		...
		if (!isFirstBuffer && audioFloatBuffer.length == floatOverlap + floatStepSize) {
			 ...
			 * i++){ audioFloatBuffer[i-floatStepSize] = audioFloatBuffer[i]; }
			 */
		}
		...
		while (!stopped && !endOfStream && totalBytesRead < bytesToRead) {
			try {
				bytesRead = audioInputStream.read(audioByteBuffer, offsetInBytes + totalBytesRead,
		...
		if (endOfStream) {
			...
			if (zeroPadLastBuffer) {
				...
				converter.toFloatArray(audioByteBuffer, offsetInBytes, audioFloatBuffer, offsetInSamples,
						floatStepSize);
			} else {
			...
			throw new IOException(String
					.format("The end of the audio stream has not been reached and the number of bytes read (%d) is not equal "
