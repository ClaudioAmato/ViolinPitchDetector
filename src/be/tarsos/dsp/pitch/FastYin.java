 ...
 * >the YIN paper.</a> This implementation is done by
 * <a href="mailto:matthias.mauch@elec.qmul.ac.uk">Matthias Mauch</a> and is
 * based on {@link Yin} which is based on the implementation found in
 * <a href="http://aubio.org">aubio</a> by Paul Brossier.
 * 
...
public final class FastYin implements PitchDetector {
	...
	private void difference(final float[] audioBuffer) {
		...
		for (int j = 0; j < audioBuffer.length; ++j) {
			...
			yinStyleACF[2 * j + 1] = audioBufferFFT[2 * j + 1] * kernel[2 * j]
					+ audioBufferFFT[2 * j] * kernel[2 * j + 1]; // imaginary
		}
