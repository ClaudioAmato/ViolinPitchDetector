...
public class FFT {
	 ...
	 *            Apply the specified window on the samples before a forward
	 *            transform. arning: the window is not applied in reverse when a
	 ...
	 *            The FFT transformed data.
	 * @param power
	 *            The array where the magnitudes or powers are going to be
	 *            stored. It is half the length of data (FFT size).
	 * @param phase
	 *            The array where the phases are going to be stored. It is half
	...
	/**
	 * Multiplies to arrays containing imaginary numbers. The data in the first
	 * argument is modified! The real part is stored at <code>2*i</code>, the
	 * imaginary part <code>2*i+i</code>
	 * 
	 * @param data
	 *            The array with imaginary numbers that is modified.
	 * @param other
	 *            The array with imaginary numbers that is not modified. Data
	 *            and other need to be the same length.
	 */
	public void multiply(float[] data, float[] other) {
		...
		for (int i = 1; i < data.length - 1; i += 2) {
			...
			// fix by perfecthu
			// data[realIndex] = data[realIndex] * other[realIndex] + -1 *
			// data[imgIndex] * other[imgIndex];
			// data[imgIndex] = data[realIndex] * other[imgIndex] +
			// data[imgIndex] * other[realIndex];
		}
	}
}
